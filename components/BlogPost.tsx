  ...
  return (
    <Link href={`/blog/${blogData.slug}`} className="w-full ">
      <div className="w-full mb-2 flex dark:bg-gray-900 border-2 border-primary shadow rounded-xl">
        <div className="w-full p-4 max-h-48">
          <div className="w-full ">
            <p className="mb-1 text-sm text-gray-500">
              {blogData.readTime} min read • <time>{publishedDate}</time> •{' '}
              {`${views ? new Number(views).toLocaleString() : '–––'}`} views
            </p>
            <h3 className="text-lg md:text-lg font-medium w-full text-gray-900 dark:text-gray-100 tracking-tight line-clamp-2">
              {blogData.title}
            </h3>
            <p className="text-gray-500 dark:text-gray-300 tracking-tight line-clamp-2">
              {blogData.excerpt}
            </p>
